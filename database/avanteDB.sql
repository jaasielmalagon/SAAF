...
-- Tiempo de generación: 16-05-2018 a las 16:28:16
...
--
DROP TRIGGER IF EXISTS `copiar_historial_domicilios`;
DELIMITER //
CREATE TRIGGER `copiar_historial_domicilios` BEFORE UPDATE ON `domicilios`
 FOR EACH ROW INSERT INTO `historial_domicilios`(`idDomicilio`, `sucursal`, `calle`, `numero`, `calle1`, `calle2`, `colonia`, `tipo`, `propietario`, `vigencia_contrato`, `tiempoResidencia`) VALUES (
...
CREATE TABLE IF NOT EXISTS `historial_domicilios` (
  `idHistorialDomicilio` int(11) NOT NULL AUTO_INCREMENT,
  ...
  `vigencia_contrato` date NOT NULL,
  `tiempoResidencia` int(11) NOT NULL,
  PRIMARY KEY (`idHistorialDomicilio`)
...
CREATE TABLE IF NOT EXISTS `historial_personas_clientes` (
  `idHistorialClientes` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`idHistorialClientes`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci AUTO_INCREMENT=1 ;
...
--
DROP TRIGGER IF EXISTS `copiar_historial_personas_clientes`;
DELIMITER //
CREATE TRIGGER `copiar_historial_personas_clientes` BEFORE UPDATE ON `personas_clientes`
...
CREATE TABLE IF NOT EXISTS `personas_staff` (
...
INSERT INTO `personas_staff` (`idStaff`, `sucursal`, `usuario`, `registro`, `idPersona`, `cargo`, `estudios`, `departamento`, `salario`, `entrada`, `salida`, `dias`, `llamara`, `fecha_incorp`, `efectivo`, `codigo`) VALUES
(1, 1, 3, '2018-05-03 13:02:43', 3, 5, 6, 8, 111, '08:00:00', '19:00:00', 'Lunes-Martes-Miércoles-Jueves-Viernes-Sábado-Domingo', 'ESPOSO 2381682735 RENAUD', '2018-04-04', 3000, '01S-A');
...
CREATE TABLE IF NOT EXISTS `personas_staff_adc` (
...
INSERT INTO `personas_staff_adc` (`idAdc`, `sucursal`, `idStaff`, `agencia`, `vacante`, `nivel`) VALUES
(1, 1, 0, 1, 1, 0),
(2, 1, 0, 1, 2, 0),
(3, 1, 0, 1, 3, 0),
(4, 1, 0, 1, 4, 0),
...
CREATE TABLE IF NOT EXISTS `prestamos_solicitudes` (
...
CREATE TABLE IF NOT EXISTS `tipo_ocupaciones` (
...
CREATE TABLE IF NOT EXISTS `tipo_status_cliente` (
